...
- [ ] Revalidate provider and resolved model after local preparation and before consent/request dispatch. `AnalysisRunner` now routes the post-preparation boundary through `ProviderGate::revalidate_model`, which performs a fresh provider handshake and adapter-owned model validation before consent; the provider contract test covers an unavailable model with zero request construction. Full workflow integration evidence remains blocked by the concurrently dirty `crates/qaptr-workflow/tests/analyze.rs`, whose new cancellation fixture currently fails to type-check because its helper accepts only `FakeConsent`; that active change was not edited.
...
- [ ] Provider/model revalidation boundary: `ProviderGate::revalidate_model` performs a fresh provider handshake and invokes an adapter-owned model-validation hook after local preparation, while `AnalysisRunner` uses its returned fresh proof before consent (`crates/qaptr-provider/src/adapter.rs:407-501`, `crates/qaptr-workflow/src/analyze.rs:389-403`). The provider contract suite passes 10/10, workflow library and session suites pass 8/8 each, and strict focused Clippy passes. The full workflow analysis suite remains blocked by the pre-existing active cancellation fixture type mismatch described above; no provider or credential data was used.
