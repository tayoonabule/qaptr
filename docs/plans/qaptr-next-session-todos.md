...
- [ ] `99bfc9d` introduced a scalar-JSON review-session driver and its initial 32-test proof, but an independent concurrency/ABI audit found P1 defects in two-pass mutation handling, worker destruction, cancellation during ingestion, transition races, and retention/durable deduplication. The driver remains deliberately unwired from Swift while a repair adds deterministic regression coverage. Do not treat this as accepted review-session orchestration yet.
