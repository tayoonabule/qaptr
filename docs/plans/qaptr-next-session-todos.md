...
- [ ] `5eb9da2` adds review-driver replay, retry, retention/history, cancellation, and consent regressions; `7e79fd5` restores clean Rust formatting for that scoped source. Fresh public-boundary evidence on 2026-08-16: `cargo fmt --manifest-path crates/qaptr-review-ffi/Cargo.toml -- --check`, strict review-FFI Clippy, and `cargo test --manifest-path crates/qaptr-review-ffi/Cargo.toml` all pass with 46/46 tests; focused workflow session/analysis tests and strict workflow Clippy pass; the native review package passes 92/92 tests both in the working checkout and a clean `git archive` checkout. An independent C-ABI/concurrency audit is still in progress, so this remains unchecked and does not claim Swift wiring, real capture, credentials, or provider acceptance.
