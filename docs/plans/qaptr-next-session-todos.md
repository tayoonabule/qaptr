...
- [x] Local packaged UI audit identified the apparent duplicate app and the 18-capture value. The packaged app and a source debug build were briefly running at the same time. The debug UI uses `DevMockData.captureProgress` under `QAPTR_DEV_MOCK_DATA=1`, where `captureCount` is hard-coded to 18; the durable packaged SQLite store had 0 captures, 0 observations, 0 workflows, and 0 notices. The debug instance was stopped and the packaged app relaunched alone. Different app bundles/build identities can report different TCC states even when the visible name is the same. A real capture remained unverified at that point.
