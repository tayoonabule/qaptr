...
- [ ] Packaged onboarding still needs a clean completion attempt after granting Screen Recording to the packaged Review identity. The observed Step 5 error is the app's generic `loadError` guard, not evidence of a successful setup; the bridge/store initialization error must be surfaced or diagnosed before claiming onboarding acceptance.
