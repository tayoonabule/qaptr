...
- [x] Extend the bridge with coarse session operations for start, progress, grant/decline consent, cancel, and retry. The provider-aware C ABI retains an opaque Rust driver, accepts bounded scalar JSON operations with replay-safe mutation tokens, and returns bounded scalar state. Observation detail, workflow generation, and export remain separate surface work.
...
- [x] Assert a successful fixture run is visible through the same review-app snapshot API as the actual app. The scalar bridge coverage verifies observation/workflow counts through snapshot and status JSON without vault ids, images, credentials, or raw provider material. The production driver now binds one selected Claude, Codex, or Jcode CLI adapter through the provider-aware open function, while deterministic coordinator coverage keeps the adapter endpoint fake and exercises the real vault, privacy, consent, normalization, and persistence boundaries. Credentialed external-provider acceptance remains open in section 6.2.
...
- [x] Surface the resolved CLI model or an honest “provider default” label in consent and result metadata. The native driver emits the immutable request-scoped model label in both ready-for-consent and terminal state, and the Swift consent view renders it without inventing a model identifier.
...
- [x] Show the selected provider, resolved model label, payload kind, prepared capture count, image count, and exclusions in the consent view. The Swift model decodes only the native driver's bounded scalar summary.
- [x] Ensure provider/model selection alone cannot invoke analysis or send a provider payload. Selection and connection checks are separate from the explicit Observation Sheet start action, and the native worker cannot invoke until its distinct consent mutation is granted.
...
- [x] Render actual review-session observations from durable history. Terminal native session state triggers a fresh snapshot read, so newly persisted scalar observations replace the prior read-only view without provider output crossing the Swift bridge.
...
- [x] Add honest analysis states for no committed captures, no safely prepared captures, no observations, declined consent, cancellation, provider failure, and successful observation writes.
...
- [ ] Run `bench/scripts/release_validate.sh` and review the timestamped report under `bench/results/`.
...
- [x] Packaged smoke evidence is generated by `bench/scripts/packaged_fixture_smoke.sh` under ignored `bench/results/` output.
...
- [ ] Production onboarding/provider audit: the package embeds the helper and exposes bounded CLI readiness plus provider-aware native analysis for selected Claude, Codex, and Jcode sessions. The driver performs fresh authentication/model revalidation after local preparation and before consent, and the Swift UI requires an explicit start followed by separate payload consent. Remaining audit gaps include clean-install helper registration/startup proof, real TCC behavior, attached-display selection, OpenRouter session wiring, credentialed provider acceptance, and external release evidence. Scoped package/FFI tests are implementation evidence, not external acceptance.
...
- [x] Local packaged UI audit identified the apparent duplicate app and the 18-capture value. At 2026-08-16 20:09 UTC, two `QaptrReview` processes were running: the packaged `/Users/light/Applications/Qaptr.app/Contents/Applications/QaptrReview.app/.../QaptrReview` and the source debug `apps/review/.build/arm64-apple-macosx/debug/QaptrReview.app/.../QaptrReview`. The debug UI uses `DevMockData.captureProgress` under `QAPTR_DEV_MOCK_DATA=1`, where `captureCount` is hard-coded to 18; the durable packaged SQLite store currently has 0 captures, 0 observations, 0 workflows, and 0 notices. The debug instance was stopped and the packaged app relaunched alone. The two processes can report different TCC states because they are different app bundles/build identities even though the visible name is the same. A real capture remains unverified.
- [ ] Packaged onboarding still needs a clean completion attempt after granting Screen Recording to the packaged Review identity. The observed Step 5 error came from the app's `loadError` guard; the packaged build now displays the underlying bridge/store error instead of only saying to reopen Qaptr (`e73c78a`, Review Swift tests 98/98). A real permission and capture acceptance attempt remains required.
- [x] Accessibility status now follows the process that actually reads app/window names. `QaptrHelper` publishes its live `AXIsProcessTrusted()` state with PID/timestamp, `QaptrReview` ignores stale/dead helper snapshots, refreshes that state after relaunch, and routes the Allow action to the helper instead of requesting the review app's unrelated TCC entry (`b937df1`; Helper Swift tests pass, Review Swift tests pass 125/125, release build passes, and signed packaging verifies the nested apps and DMG). A live user permission toggle still needs to be exercised on the freshly packaged build.
- [x] Provider-aware review-session wiring now reaches the real just-in-time consent boundary in the signed local Review app. On 2026-08-20, the selected authenticated Jcode CLI prepared 38 existing committed captures locally, then displayed `jcode`, the honest `provider default` model label, `Text only`, 38 prepared captures, 0 images sent, and 0 local exclusions. The request was declined with **Keep local**, and the terminal UI confirmed **Nothing was sent**. This validates real vault decoding, local preparation, fresh CLI verification, scalar Swift/C state, and the zero-dispatch decline path. It deliberately does not close credentialed provider invocation row 191. Full Rust formatting/Clippy/workspace tests/docs, both Swift suites, the signed package build, and packaged fixture smoke also passed in the same checkout.
- [x] Website five-card responsive polish shipped in `6459712`. The Astro check reported 0 errors, the production build passed, and the browser suite passed 9/9 including responsive coverage at 320, 640, 768, and 1440px. After pushing `main`, `https://qaptr.headless.com` served the new `index.CoDZPndx.css` asset; public Playwright checks at 320, 390, 640, 768, and 1440px observed five separated cards, no horizontal overflow, the four intended feature fills, and working `/#join` navigation with the `/api/waitlist` form visible. No real waitlist submission was made during acceptance.
- [x] Website card graphics received a second Impeccable polish pass in `web/src/pages/index.astro`: the session ledger, capture timeline, privacy orbit, review document, and handoff graphic now share a measured internal rhythm with aligned rows, centered geometry, stable mobile sizing, and no detector findings. Astro check/build passed, accessibility passed, the responsive E2E suite passed 9/9 on rerun, and rendered Playwright captures were checked at 1440px and 390px with no horizontal overflow. The capture timeline dots were then measured against the line center and corrected to within 0.1px at both 1440px and 390px.
- [x] Create and wire a production social share image for the website, using the Qaptr visual system and the standard 1200×630 Open Graph canvas. Added a funnel-aware Hook/Story/Offer composition, Open Graph and Twitter card metadata, and verified the rendered SVG has no overlap or clipping at 1200×630.
