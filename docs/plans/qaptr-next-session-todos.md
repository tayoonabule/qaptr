...
- [x] `qaptr-review-ffi` now exposes an app-owned, versioned scalar-JSON session handle with bounded `start`, `state`, `decide_consent`, `cancel`, and `retry` operations. It discovers committed bundles through the vault API, uses local decoder/privacy seams, reports only scalar counts and consent metadata, and deliberately fails provider selection closed until a real adapter is wired. Independently rerun `cargo test --manifest-path crates/qaptr-review-ffi/Cargo.toml` passes 32 tests, including malformed JSON, missing-bundle, consent grant/decline/cancel, retry, and no-image assertions; strict clippy and formatting pass (`99bfc9d`). Swift bridge/UI wiring and real-provider analysis remain open.
