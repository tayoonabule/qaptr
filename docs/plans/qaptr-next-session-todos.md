...
- [ ] `99bfc9d` introduced a scalar-JSON review-session driver and its initial 32-test proof. The first repair (`8b5c8aa`, formatted in `03ea616`) passes 37 focused tests, but an independent re-audit remains **NO-GO** for Swift wiring: interleaved two-pass mutation reads can re-execute a request, cancellation can be acknowledged before its handle is published, invalid start/retry can clear pending consent, and production coordination still omits retention plus durable-history deduplication. The driver remains deliberately unwired from Swift until those four P1 regressions are fixed and re-reviewed. Do not treat this as accepted review-session orchestration yet.
