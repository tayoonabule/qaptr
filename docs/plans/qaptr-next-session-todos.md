...
- [ ] `5eb9da2` adds review-driver replay, retry, retention/history, cancellation, and consent regressions; `7e79fd5` restores clean Rust formatting for that scoped source. Fresh public-boundary evidence on 2026-08-16: `cargo fmt --manifest-path crates/qaptr-review-ffi/Cargo.toml -- --check`, strict review-FFI Clippy, and `cargo test --manifest-path crates/qaptr-review-ffi/Cargo.toml` all pass with 46/46 tests; focused workflow session/analysis tests and strict workflow Clippy pass; the native review package passes 92/92 tests both in the working checkout and a clean `git archive` checkout. Independent re-audit is **NO-GO**: the replay boundary needed an explicit request token, and retry incorrectly treated failed/incomplete captures as complete after SQLite reopen (`crates/qaptr-review-ffi/src/driver.rs:60-68,720-784`; `crates/qaptr-workflow/src/session.rs:283-313,360-393`). The workflow side is repaired in this pass: durable capture metadata is no longer treated as completion evidence, and `restarted_coordinator_does_not_treat_failed_capture_metadata_as_complete` passes with 8/8 session tests. Review-FFI token work is concurrently dirty and not revalidated here; the row remains unchecked. This row does not claim Swift wiring, real capture, credentials, or provider acceptance.
