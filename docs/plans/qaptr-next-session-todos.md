...
- [ ] `55ada11` expands review-driver regression coverage to 40 FFI tests and adds retention/history tests. The current uncommitted follow-up work was revalidated on 2026-08-15 through the public review FFI boundary: `cargo test --manifest-path crates/qaptr-review-ffi/Cargo.toml` passed 44/44, and the workflow session/analysis suites passed 19/19. The native review package passed 92/92 Swift tests. `cargo fmt --check` still fails on active, uncommitted formatting changes in `crates/qaptr-workflow/tests/session.rs`. These results improve the evidence for the P1 fixes but do not constitute independent acceptance or Swift wiring acceptance; the driver remains deliberately unwired until the active code is committed, formatting is clean, C-ABI behavior is independently re-reviewed, and the full public path is accepted. No external permission, credential, capture, or provider evidence was claimed.
