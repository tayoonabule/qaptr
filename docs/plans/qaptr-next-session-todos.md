...
- [ ] `55ada11` expands review-driver regression coverage to 40 FFI tests and adds retention/history tests, but a second independent audit remains **NO-GO** for Swift wiring: identical interleaved two-pass mutations can still consume each other's raw-request cache entry and re-execute, while durable-history filtering makes production retry unable to reprocess captures after a transient failure. The driver remains deliberately unwired from Swift until both P1 regressions are fixed, C-ABI reproduced, and independently re-reviewed. Do not treat this as accepted review-session orchestration yet. Focused validation of the current uncommitted follow-up work on 2026-08-15 ran 44 bridge tests with 42 passing and 2 failing (`interleaved_distinct_two_pass_mutations_keep_each_response`, `identical_interleaved_cancel_passes_keep_terminal_rejection_owned`); the workflow retry/analysis suites passed 19 tests. No acceptance claim is made, and the code remains an active dirty worktree owned by the in-progress fix rather than this session.
...
- [ ] Native Review UI/package audit is **NO-GO**: clean HEAD cannot compile because committed `SettingsView` references `InstalledApplication`/`ApplicationCatalog` while `ApplicationCatalog.swift` is untracked, and native Settings entry points can bypass onboarding/privacy/capture-consent gates. Keep packaging blocked until the missing source is committed with focused clean-checkout tests and Settings is gated by the same onboarding requirements.
- [ ] Production onboarding/provider audit: the package embeds the helper, but onboarding can complete before helper registration/startup is proven; capture permission is requested for the Review identity while the helper separately preflights its own permission; attached displays are treated as selected; provider verification can run before the final privacy stage; CLI readiness has no production surface; and the packaged bridge remains read-only with `UnavailableProvider`, so post-onboarding analysis is unavailable. Scoped test evidence passes (89 Review Swift tests, 37 review-FFI tests, package dry run and fixture smoke), but these are implementation blockers, not completion evidence.
