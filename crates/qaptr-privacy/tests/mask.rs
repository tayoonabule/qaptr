...
use qaptr_privacy::{
    DetectionKind, Image, ImageOrientation, MASK_COLOR, MappedDetection, map_normalized_rect,
    mask_image,
};
...
fn every_detected_region_is_covered_in_output_pixels() {
    ...
    let masked = mask_image(&source, &detections).expect("masking should succeed");
...
fn edge_touching_detection_is_fully_covered_without_writing_outside_image() {
    ...
    let masked = mask_image(&source, &[edge]).expect("edge masking should succeed");

    assert_rect_is_masked(masked.image(), edge);
    assert_eq!(masked.image().pixel(3, 0), Some([200, 201, 202]));
    assert!(masked.verify(&[edge]).is_ok());
}
...
fn overlapping_detections_leave_no_gap_and_are_each_proven() {
    ...
    let masked = mask_image(&source, &[left, right]).expect("overlap masking should succeed");
    ...
    assert!(masked.verify(&[left, right]).is_ok());
}
...
fn masking_is_deterministic_for_same_pixels_and_detections() {
    ...
    let first = mask_image(&source, &detections).expect("first masking should succeed");
    let second = mask_image(&source, &detections).expect("second masking should succeed");
...
fn no_detections_preserves_pixels_and_still_returns_a_valid_proof() {
    ...
    let masked = mask_image(&source, &[]).expect("no-op masking should succeed");
