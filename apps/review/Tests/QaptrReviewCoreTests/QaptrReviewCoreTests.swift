...
final class OnboardingStageTests: XCTestCase {
  func testAdvancesThroughTheTwoPermissionStagesInOrder() {
    var stage: OnboardingStage? = .screenRecording
    var seen: [OnboardingStage] = []
    while let current = stage {
      seen.append(current)
      stage = current.next
    }
    XCTAssertEqual(seen, [.screenRecording, .accessibilityContext])
  }

  func testOptionalAccessibilityIsTheTerminalStage() {
    XCTAssertNil(OnboardingStage.accessibilityContext.next)
  }
}
...
final class ReviewFFILibraryPathTests: XCTestCase {
  ...
  func testSecureBootstrapUsesTheVaultBesideDurableHistory() {
    let store = URL(
      fileURLWithPath: "/Users/example/Library/Application Support/Qaptr/history.sqlite3")
    XCTAssertEqual(
...
final class OnboardingCopyTests: XCTestCase {
  func testPermissionFlowExplainsImmediateCaptureAndOptionalContext() {
    XCTAssertTrue(OnboardingCopy.screenRecordingStep.contains("required"))
    XCTAssertTrue(OnboardingCopy.accessibilityStep.contains("Optional"))
    XCTAssertTrue(OnboardingCopy.captureBeginsAfterPermission.contains("immediately"))
  }

  func testDetailedSessionDurationHasBoundedChoices() {
    XCTAssertEqual(
      DetailedSessionDuration.allCases.map(\.seconds),
      [1_800, 3_600, 7_200]
    )
    XCTAssertEqual(DetailedSessionDuration.oneHour.displayName, "1 hour")
  }
